...
function initAnimations() {
  ...
  heroTimeline
    ...
    .to(
      ".reveal-title",
      { opacity: 1, y: 0, scale: 1, filter: "blur(0px)", duration: 1.15 },
      "-=0.72",
    );
  ...
  gsap.utils
    .toArray(
      ".package-mini-card, .demo-card, .process-card, .faq-item, .problem-card, .include-feature-card, .mobile-proof-list article, .mobile-feature-phone, .mobile-proof-card, .lived-experience-card, .lived-experience-closing",
    )
    .forEach((card, index) => {
      gsap.fromTo(
        card,
...
function updateExtrasByPackage() {
  ...
  options.forEach((option) => {
    ...
    const allowedPackages = (option.dataset.packages || "")
      ...
      .filter(Boolean);
    const isAllowed =
      selectedPackage && allowedPackages.includes(selectedPackage);
  ...
  if (!selectedPackage) {
    hint.textContent =
      "Selecciona primero un paquete para ver qué adicionales aplican.";
    return;
  }
  ...
  hint.textContent =
    "Selecciona los adicionales que te interesen o marca “Aún no lo sé”.";
}
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.status === "success") {
        showFormFeedback(
          "Solicitud registrada. Abriendo WhatsApp para continuar la cotización...",
          "success",
        );
        setTimeout(() => {
          window.open(
            buildWhatsAppUrl(buildWhatsappMessage(leadData)),
            "_blank",
          );
          resetQuoteForm(form);
          clearQuoteDraft();
