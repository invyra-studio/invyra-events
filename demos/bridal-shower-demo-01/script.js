...
function buildWhatsAppUrl(data) {
  const lines = [
    "Hola, confirmo mi respuesta para el Bridal Shower de Renata:",
    "",
    `Nombre: ${data.name}`,
    `Respuesta: ${data.attendance}`
  ];
  if (data.attendance === "Asistiré" && data.song) lines.push(`Canción sugerida: ${data.song}`);
  if (data.message) lines.push(`Nota para Renata: ${data.message}`);
...
function initRsvp() {
  ...
  form.addEventListener("submit", event => {
    ...
    saveDraft();
    window.open(buildWhatsAppUrl({ name, attendance, song, message }), "_blank", "noopener,noreferrer");
  });
}
