...
const storageKey = "invyra_anniversary_demo_01_rsvp";
...
function saveDraft() {
  ...
  localStorage.setItem(storageKey, JSON.stringify({
    name: data.get("guestName") || document.getElementById("guestName").value,
...
function updateAttendanceState() {
  ...
  const isDeclined = attendance === "No asistiré";
  messageField.placeholder = isDeclined ? "Puedes dejar un mensaje opcional" : "Mensaje opcional para Sofia y Daniel";
}
...
function submitRsvp(event) {
  ...
  const message = `Hola INVYRA, confirmo RSVP para Brindis de Oro.%0AInvitado: ${encodeURIComponent(name)}%0ARespuesta: ${encodeURIComponent(attendance)}`;
